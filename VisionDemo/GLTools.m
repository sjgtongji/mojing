...
- (GLDiscernPointModel *)handlerFaceLandMark:(NSArray *)observations image:(UIImage *)image
{
    ...
    for (VNFaceObservation *faceObservation in observations) {
        ...
        [self getKeysWithClass:[VNFaceLandmarks2D class] block:^(NSString *key) {
            ...
            if ([key isEqualToString:@"leftEye"]) {
                pointModel.leftEyePoints=sPoints;
            }
            if ([key isEqualToString:@"rightEye"]) {
                pointModel.rightEyePoints=sPoints;
            }
            if ([key isEqualToString:@"leftEyebrow"]) {
                pointModel.leftEyebrowPoints=sPoints;
            }
            if ([key isEqualToString:@"rightEyebrow"]) {
                pointModel.rightEyebrowPoints=sPoints;
            }
            if ([key isEqualToString:@"leftPupil"]) {
                pointModel.leftPupilPoints=sPoints;
            }
            if ([key isEqualToString:@"rightPupil"]) {
                pointModel.rightPupilPoints=sPoints;
            }
            if ([key isEqualToString:@"outerLips"]) {
                pointModel.outerLipsPoints=sPoints;
            }
            if ([key isEqualToString:@"innerLips"]) {
                pointModel.innerLipsPoints=sPoints;
            }
            if ([key isEqualToString:@"faceContour"]) {
                pointModel.faceContourPoints=sPoints;
            }
